...
window.JOSH = (function () {
  ...
  /* Scene frame:
       shell = enter(frame, fps)                  -> top bar settles in
       exit  = clamp(frame,[dur-12,dur-1],[1,0])  -> page fades out

     The source's ambient blob (a blurred red circle pulsing on
     sin(frame/34)) is NOT carried over. It was a dark-UI device; on a
     light page it reads as a smudge rather than depth. The page's own
     blue blooms replace it and stay static. */
  function shell(tl, totalFrames) {
    ...
    tl.to("#fill", { opacity: 0, duration: T(11), ease: "none" }, T(totalFrames - 12));
  }
