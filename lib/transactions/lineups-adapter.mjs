...
function expectedState(manifest, ownerId) {
  if (ownerId === "figma-scene") {
    return {
      ...
      proofFrames: manifest.export.motionProof.frames,
  ...
  if (ownerId === "eagle-render") {
    return {
      ...
      motionEngine: manifest.motion.engine,
      cueSha256: hashValue(manifest.motion.cues),
      artifactRole: manifest.export.artifactRole,
      backgroundPolicy: manifest.export.backgroundPolicy,
      sourceRevision: manifest.figma.sourceRevision,
      proofFrames: manifest.export.motionProof.frames,
    };
  }
  ...
  return {
    ...
    duration: manifest.premiere.approvedDuration,
    endTime: manifest.premiere.approvedEndTime,
  };
}
