...
export default function AiWorkflowSessionForm() {
  ...
  return <form onSubmit={submit} className="space-y-9" encType="multipart/form-data">
    ...
    <div><label className="block"><span className="text-xl font-semibold tracking-[-0.025em] text-neutral-950">Add a Loom link, attach a voice memo, or paste your notes.</span><textarea name="aiWish" rows={4} aria-label="Add a Loom link, attach a voice memo, or paste your notes" className="mt-4 block w-full resize-y rounded-2xl border border-neutral-300 bg-white px-4 py-3 text-[16px] leading-6 text-neutral-950 outline-none focus:border-neutral-950 focus:ring-2 focus:ring-neutral-950/10" /></label><label className="mt-3 inline-block cursor-pointer text-sm font-medium text-neutral-600 underline decoration-neutral-300 underline-offset-4 transition hover:text-neutral-950 hover:decoration-neutral-950"><span>Attach a voice memo</span><span className="sr-only"> (optional, up to 25 MB)</span><input name="audio" type="file" accept="audio/*" className="sr-only" /></label></div>
