...
type UiMessage =
  ...
  | { type: "insert"; item: SearchItem; mode: "insert" }
  | { type: "insert"; item: SearchItem; mode: "replace"; targetId: string };

type PluginMessage =
  | { type: "selection"; payload: SelectionSnapshot }
  | { type: "working"; message: string }
  | { type: "success"; message: string; nodeId: string; eagleItemId: string }
  | { type: "error"; message: string };

const BRIDGE_BASE = "http://localhost:41723";
const EAGLE_ID_KEY = "lineupsEagleItemId";
let currentSnapshot: SelectionSnapshot = emptySnapshot();
...
function imagePaints(node: SceneNode): ImagePaint[] {
...
function fillTargetsFor(selection: readonly SceneNode[]): SceneNode[] {
...
function canInsertInto(node: SceneNode | undefined): node is FrameNode | ComponentNode {
  return Boolean(node && (node.type === "FRAME" || node.type === "COMPONENT"));
}

function suggestedFillTarget(targets: SceneNode[]): SceneNode | null {
  ...
  return (
    targets.find((node) => personPattern.test(node.name) && !backgroundPattern.test(node.name)) ??
    targets.find((node) => !backgroundPattern.test(node.name)) ??
    targets[0] ??
    null
  );
}

function selectionData(): SelectionSnapshot {
...
async function fetchEagleImage(itemId: string): Promise<Image> {
...
function replaceImageFill(node: SceneNode, imageHash: string): void {
  ...
  fills[index] = { ...(fills[index] as ImagePaint), imageHash, visible: true };
  node.fills = fills;
}

async function insertNewLayer(item: SearchItem, image: Image): Promise<RectangleNode> {
...
async function replaceLayer(item: SearchItem, image: Image, targetId: string): Promise<SceneNode> {
...
async function handleInsert(message: Extract<UiMessage, { type: "insert" }>): Promise<void> {
  post({ type: "working", message: `Loading ${message.item.person} from Eagle…` });
  const image = await fetchEagleImage(message.item.id);
  const node = message.mode === "replace"
    ? await replaceLayer(message.item, image, message.targetId)
    : await insertNewLayer(message.item, image);
