# White-Collar Work History Application Ledger

Use this when an application asks for white-collar work-history, education, or
basic profile fields. These are paste-ready values from
`white-collar_Jerami_Singleton_Resume2026.pdf`.

## Profile

```text
Name: Jerami Singleton
Email: Jay23singleton@gmail.com
Phone: 407-473-3637
City: Riverview
State: FL
Portfolio: https://jeramisingleton.carrd.co/
LinkedIn: https://www.linkedin.com/in/jeramisingleton/
Work Authorization: Authorized to work in the United States; no sponsorship required.
Availability / Start Date: As soon as possible
Desired Pay - Cash Jobs: $21+/hour
Desired Pay - Career Jobs: $30+/hour
Address note: Full street address is not stored in the resume source.
Personal address rule: Do not store a street address here; forms can handle this manually.
```

## National Prospect ID

```text
Company Name: National Prospect ID
Position: Video Editor
City/State: Remote
Start Date: 02/2024
End Date: 06/2026
Currently Working Here: No
Address Line 1: 18291 N Pima Rd
Address Line 2: Suite 110-379
City: Scottsdale
County: Maricopa
State: AZ
Zip Code: 85255
Phone: 844-500-0622
Reason for Leaving: Contract ended
Responsibilities: Produced 1,000+ multi-sport athlete highlight reels for recruiting distribution; scaled output to 70 videos per month during peak enrollment growth; built structured post-production systems with templates, naming conventions, and QC workflows; implemented a FastAPI translator layer to stabilize legacy platform workflows; and maintained consistent weekly delivery under enrollment spikes.
Reference: James Holcomb
Reference email: jholcomb@prospectid.com
Reference phone: 480-340-4559
```

## NurseHub

```text
Company Name: NurseHub
Position: Video Editor
City/State: Remote
Start Date: 07/2024
End Date: 12/2025
Currently Working Here: No
Address Line 1: 6 Liberty Square
Address Line 2: #517
City: Boston
County: Suffolk
State: MA
Zip Code: 02109
Phone:
Reason for Leaving: Contract backlog work ended
Responsibilities: Restructured roughly 38 hours of curriculum; processed 180-200 lesson assets; led most of the migration through deterministic FFmpeg workflows; increased course assembly throughput 2-3x; eliminated export errors; standardized naming schemas; consolidated transcripts; replaced manual timelines with automated batch export pipelines; and established scalable folder architecture and encoding standards.
Reference: Caroline Dobrez
Reference email: caroline.dobrez@nursehub.com
Reference phone:
Reference note: Add phone if needed.
```

## Home Shopping Network

```text
Company Name: Home Shopping Network
Position: Production Technician
City/State: St. Petersburg, Florida
Start Date: 12/2020
End Date: 02/2023
Currently Working Here: No
Address Line 1: 1 HSN Drive
Address Line 2:
City: St. Petersburg
County: Pinellas
State: FL
Zip Code: 33729
Phone: 727-872-1000
Reason for Leaving: Role ended
Responsibilities: Supported high-volume retail television content creation through on-set production, camera operation, lighting, and asset capture; worked directly with models, products, and creative teams to produce demo, application, and before/after footage for broadcast and digital use; and ensured visual consistency, brand standards, and technical quality across large-scale product shoots.
Reference note: Add reference name, email, and phone if needed.
```

## WFLA News Channel 8

```text
Company Name: WFLA News Channel 8
Position: Production Technician
City/State: Tampa, Florida
Start Date: 01/2018
End Date: 04/2019
Currently Working Here: No
Address Line 1: 200 S Parker St
Address Line 2:
City: Tampa
County: Hillsborough
State: FL
Zip Code: 33606
Phone: 813-228-7777
Reason for Leaving: Role ended
Responsibilities: Assisted in live news broadcasts, coordinated studio floor activity with producers and directors under live production constraints, operated graphics systems and robotic cameras, and supported reliable on-air production.
Reference note: Add reference name, email, and phone if needed.
```

## Freelance Broadcast Production Assistant

```text
Company Name: Freelance
Position: Broadcast Production Assistant
City/State: Central Florida
Start Date: 08/2014
End Date: 01/2018
Currently Working Here: No
Address Line 1:
Address Line 2:
City:
County:
State: FL
Zip Code:
Phone:
Reason for Leaving: Role ended
Responsibilities: Supported camera, graphics, and technical operations for live sports broadcasts across collegiate and professional events; assisted with setup, operation, and breakdown of broadcast equipment; and supported live production workflows.
Address note: Use only when freelance history is accepted as an employer entry.
```

## Stetson University

```text
School: Stetson University
Degree: Bachelor of Communications
Field of Study: Communications
City: DeLand
State: FL
Graduation Year: 2016
Start Year:
Start Year: 2012
Graduation Date: May 7, 2016
End Year: 2016
Reference: Jeff Taylor
Reference email: jefft7@gmail.com
Reference phone: 407-766-9381
```

## Sources Checked

```text
HSN:
https://www.sec.gov/Archives/edgar/data/1434729/000143472917000004/hsni-123116x10kdoc.htm

WFLA:
https://www.tampabaychamber.com/membership/partner/wfla-tv-nexstar-media-inc/

National Prospect ID:
https://www.bbb.org/us/az/scottsdale/profile/technical-recruiting/national-prospect-id-llc-1126-1000046111
https://www.linkedin.com/company/prospect-id-28

NurseHub:
https://nursehub.com/terms/
```
