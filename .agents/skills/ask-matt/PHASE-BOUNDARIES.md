...
The **phase boundary** is the gap between two phases, and it is the only place this decision belongs. Mid-phase there is no decision to make — continue, or prepare a handoff for work the user will launch separately. Compacting mid-phase makes the agent lose the thread.
...
| **New task** | Write a handoff; the user launches the separate task.              |
...
- swapping to a **new model or harness**,
...
**4. Should the work move to a separate task?** If it is tightly scoped and this session should remain untouched, write a `handoff` document. Stop there; the user decides whether and where to launch it.
