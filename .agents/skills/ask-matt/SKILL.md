---
name: ask-matt
description: "Route a request to the right Matt Pocock skill or flow. Use only when the user explicitly names this skill."
---
...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`to-spec`** (turn the thread into a spec), then **`to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links. The user chooses and invokes **`implement`** for each ticket.
   - **No** → **`implement`** right here, in the same context window.

   `implement` starts with one tracer bullet — the thinnest complete user-visible path — and verifies it before widening the change. Larger work continues one slice at a time in fresh `implement` sessions. It applies the `tdd` red-green method internally, then recommends `code-review` as the next explicit skill invocation before any requested commit.
...
Two reference skills sit beneath the other workflows as sources of vocabulary. Invoke either by name when the **words**, not the process, are the problem.
...
- **New task** — prepare a handoff, then let the user launch a tightly-scoped task in another window.
...
Read [PHASE-BOUNDARIES.md](PHASE-BOUNDARIES.md) for the ordered tree and the reasoning behind each branch. Make the decision **at** a boundary; mid-phase, continue or prepare a handoff for a user-launched task.
...
- **`research`** — investigate a question against **primary sources** in the current session, then leave a cited Markdown file in the repo. The file it produces feeds the thinking in `grill-with-docs`; it does not replace it.
...
- **`wizard`** — for the steps only a **human** can take: provisioning infrastructure, credentials, CI secrets, unfamiliar dashboards, or a one-off migration. Recommend it and let the user invoke it.
